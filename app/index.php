...
<div id="chart-overlay" class="overlay" style="display: none;">
    <div class="modal">
        <button id="close-chart-button" class="close-button">×</button>
 ...
 * 
 * Die Query verwendet folgende Logik:
 *  - Einzelbuchungen (recurring = 'no') des Quellmonats werden berücksichtigt.
 *  - Wiederkehrende Einträge (recurring != 'no') werden nur einbezogen, wenn:
 *       * sie entweder für alle Monate (recurring_in_month = '0') oder explizit
 *         für den aktuellen Monat gelten (FIND_IN_SET).
 *       * ihr Startzeitpunkt bereits erreicht ist ((entry_year < ? OR (entry_year = ? AND entry_month <= ?))).
 *       * und ihr Enddatum entweder '00.0000' (unbegrenzt) oder noch nicht abgelaufen ist,
 *         basierend auf end_date, das in Monat und Jahr zerlegt wird.
 *       * Zusätzlich werden diese Einträge ausgeschlossen, falls ein Override existiert:
 *         (id NOT IN (SELECT override_id FROM ...))
 *  - Override-Einträge (override = 1) werden zusätzlich berücksichtigt.
 */
function calculateAndInsertCarryover($conn, $username, $m, $y, $translations) {
    // 1) Summe im Quellmonat berechnen – hier mit der neuen end_date-Logik und Override-Filter:
    ...
    ");
    // Platzhalter in der Reihenfolge:
    // 1) $m, 2) $y, 3) $m, 4) $y, 5) $y, 6) $m, 7) $y, 8) $y, 9) $m,
    // 10) $m, 11) $y, 12) $m, 13) $y
    $stmtCarry->bind_param(
        ...
        $y,         // (5) entry_year = ?
        $m,         // (6) entry_month <= ?
        $y,         // (7) Für CAST: > ? (Enddatum-Jahr)
        $y,         // (8) Für CAST: = ? (Enddatum-Jahr)
        $m,         // (9) Für CAST: >= ? (Enddatum-Monat)
        $m,         // (10) Override-Subquery: entry_month
        $y,         // (11) Override-Subquery: entry_year
        $m,         // (12) Override-Zweig: entry_month
        $y          // (13) Override-Zweig: entry_year
    );
...
// Falls der Zielmonat in der Zukunft liegt, müssen alle dazwischenliegenden Monate abgearbeitet werden:
...
// Abfrage der Overrides für den aktuellen (gewählten) Monat
$sqlOverrides = "
SELECT * 
FROM `$username`
WHERE override = 1 AND entry_month = ? AND entry_year = ?
...
WHERE 
    ...
    AND override = 0
    AND id NOT IN (
        SELECT id FROM `$username` WHERE hidden_override = 1 AND entry_month = ? AND entry_year = ?
        UNION
        SELECT override_id FROM `$username` WHERE hidden_override = 1 AND entry_month = ? AND entry_year = ?
    )
    AND (
        (recurring = 'no' AND entry_month = ? AND entry_year = ?)
        OR (
...
$stmtEntries->bind_param(
    ...
    $month,      // 14. Overrides - Monat
    $year        // 15. Overrides - Jahr
);
$stmtEntries->execute();
...
// Serien mit Overrides überschreiben
$sqlHiddenEntries = "
SELECT override_id 
FROM `$username`
WHERE hidden_override = 1 AND entry_month = ? AND entry_year = ?
";
...
while ($row = $resultHidden->fetch_assoc()) {
    $hiddenEntries[] = $row['override_id']; // Nur die IDs speichern
}
$stmtHidden->close();

// Zusammenführen von Overrides/Originalen
$finalEntries = [];
foreach ($entries as $entry) {
    // Prüfen, ob der Eintrag ausgeblendet ist
    if (in_array($entry['id'], $hiddenEntries)) {
        continue; // Überspringen, wenn der Eintrag ausgeblendet ist
    }

    // Prüfen, ob ein Override existiert
    if (isset($overrides[$entry['id']])) {
        $finalEntries[] = $overrides[$entry['id']]; // Override verwenden
    } else {
        $finalEntries[] = $entry; // Original verwenden
    }
}

// Einträge ausgeben (wie in deinem Code)
foreach ($finalEntries as $row) {
    ...
    if (!is_null($row['repeat_until_year']) && !is_null($row['repeat_until_month'])) {
        if (
            ...
            continue; // Überspringen, wenn das Enddatum überschritten ist
        }
    }

    // Klassen je nach Typ zuweisen (income, expense, rebooking)
    ...
    // Dynamisches Symbol für einzeln bearbeitete Serienelemente (Overrides)
    if ($row['override'] == 1) {
        echo "<div class='recurring-icon' title='Serie'><i class='fas fa-sync-alt'></i></div>";
    ...
    echo "<div class='entry-content'>";
        ...
        // Sprachanpassung (de/en) – Beispiel
        ...
        if ($row['recurring'] !== 'no') {
            if (!is_null($row['repeat_until_year']) && !is_null($row['repeat_until_month'])) {
                echo "<div class='entry-end'>Bis " . $monthNames[$row['repeat_until_month']] . " " . $row['repeat_until_year'] . "</div>";
            } else {
        ...
        echo "<div class='entry-id'>";
            if (!empty($row['category'])) {
                echo "<span class='entry-category-id' data-category='" . htmlspecialchars($row['category']) . "' onclick='highlightCategory(this)'>" 
        ...
        if ($row['description'] !== $translations['carryover']) {
            echo "<div class='entry-actions'>";
                ...
                // Löschen
                echo "<button class='delete-button' title='" . htmlspecialchars($deleteLabel) . "' data-id='" . $row['id'] . "' data-table='" . htmlspecialchars($username) . "'><i class='fas fa-trash'></i></button>";

                // Unterscheidung Override / Umbuchung / Normal
                if ($row['override'] == 1 && $row['hidden_override'] == 0) {
                    // Bearbeitetes Serienelement
                    echo "<button class='edit-button' title='" . htmlspecialchars($edit_editedLabel) . "' data-id='" . $row['id'] . "' data-type='" . $row['type'] . "' data-edit-mode='series'><i class='fas fa-edit'></i></button>";
                    echo "<button class='hide-override-button' title='" . htmlspecialchars($hide_editedLabel) . "' data-id='" . $row['id'] . "'><i class='fas fa-eye-slash'></i></button>";
                } elseif ($row['rebooking_id'] > 0) {
                    // Umbuchung bearbeiten
                    echo "<button class='rebooking-edit-button' title='" . htmlspecialchars($edit_rebookingLabel) . "' data-id='" . $row['id'] . "' onclick='handleRebookingEdit(this)'><i class='fas fa-edit'></i></button>";

                    if ($row['recurring'] != 'no') {
                        echo "<button class='edit-single-rebooking-button' title='" . htmlspecialchars($edit_single_rebookingLabel) . "' data-id='" . $row['id'] . "' data-type='" . $row['type'] . "' data-edit-mode='single-rebooking'><i class='fas fa-edit'></i></button>";
                        if ($row['hidden_override'] == 0) {
                            echo "<button class='hide-single-button' title='" . htmlspecialchars($hide_single_rebookingLabel) . "' data-id='" . $row['id'] . "' data-month='" . $month . "' data-year='" . $year . "'><i class='fas fa-eye-slash'></i></button>";
                        }
                    } else {
                        echo "<button class='hide-button' title='" . htmlspecialchars($hide_rebookingLabel) . "' data-id='" . $row['id'] . "'><i class='fas fa-eye-slash'></i></button>";
                    }
                } else {
                    // Normaler Eintrag
                    echo "<button class='edit-button' title='" . htmlspecialchars($editLabel) . "' data-id='" . $row['id'] . "' data-type='" . $row['type'] . "' data-edit-mode='series'><i class='fas fa-edit'></i></button>";
                    if ($row['recurring'] != 'no') {
                        echo "<button class='edit-single-button' title='" . htmlspecialchars($edit_single_entryLabel) . "' data-id='" . $row['id'] . "' data-type='" . $row['type'] . "' data-edit-mode='single'><i class='fas fa-edit'></i></button>";
                        if ($row['hidden_override'] == 0) {
                            echo "<button class='hide-single-button' title='" . htmlspecialchars($hide_single_entryLabel) . "' data-id='" . $row['id'] . "' data-month='" . $month . "' data-year='" . $year . "'><i class='fas fa-eye-slash'></i></button>";
                        }
                    } else {
                        echo "<button class='hide-button' title='" . htmlspecialchars($hideLabel) . "' data-id='" . $row['id'] . "'><i class='fas fa-eye-slash'></i></button>";
                    }
                }
...
// -----------------------------------------------------------
// 4) GESAMTSUMME FÜR DEN GEWÄHLTEN MONAT BERECHNEN
// -----------------------------------------------------------
...
<script>
document.getElementById("rebooking-form").addEventListener("submit", function (event) {
    ...
    fetch("add_rebooking.php", {
        ...
        .then(data => {
            if (data.error) {
                ...
                alert(data.success); // Erfolgsmeldung anzeigen
