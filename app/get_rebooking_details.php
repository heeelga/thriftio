...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    try {
        ...
        if (!is_null($entry['override_id'])) {
            $stmt = $pdo->prepare("SELECT id, rebooking_id, rebooking_partner, type, amount, description, entry_month, entry_year, recurring, repeat_until_month, repeat_until_year, end_date FROM `$username` WHERE id = ?");
            $stmt->execute([$entry['override_id']]);
